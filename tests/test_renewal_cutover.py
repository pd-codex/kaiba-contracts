...
from tools.renewal import validate_cutover,validate_staged

class CutoverTests(unittest.TestCase):
    def setUp(self):
        def ex(name):return load(ROOT/'examples/valid'/('pilot-'+name+'.json'))
        self.a=ex('renewal-authorization-a');self.b=ex('binding-a-active')
        self.s=ex('renewal-staged-a');self.r=ex('renewal-installed-a');self.n=ex('renewal-active-a')
    ...
    def check(self):
        return validate_cutover(self.a,self.b,self.s,self.r,self.n,self.adoption,self.policy,self.decision,
            checked_at=self.now,predecessor_certificate_digest='sha256:'+'a'*64,predecessor_credential_revision=1)
    ...
    def test_predecessor_expired_at_cutover(self):
    ...
    def test_generic_transition_cannot_replace_renewal_identity(self):
        for field in ['credential_revision','certificate_digest','renewal_authorization_ref','predecessor_binding_ref']:
